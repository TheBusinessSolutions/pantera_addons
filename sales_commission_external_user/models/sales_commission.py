    ...
    def _compute_amount_company_currency(self):
        for rec in self:
            if rec.src_order_id:
                ...
                rec.amount_company_currency = rec.src_order_id.currency_id._convert(rec.amount, rec.currency_id)
            if rec.src_invoice_id:
                # rec.amount_company_currency = rec.src_invoice_id.currency_id.compute(rec.amount, rec.currency_id)
                rec.amount_company_currency = rec.src_invoice_id.currency_id._convert(rec.amount, rec.currency_id)
            if rec.src_payment_id:
                # rec.amount_company_currency = rec.src_payment_id.currency_id.compute(rec.amount, rec.currency_id)
                rec.amount_company_currency = rec.src_payment_id.currency_id._convert(rec.amount, rec.currency_id)
    #@api.multi
    @api.depends('amount','currency_id', 'src_order_id', 'src_invoice_id', 'src_payment_id')
